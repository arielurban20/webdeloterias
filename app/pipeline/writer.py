...
def save_draw(db: Session, normalized: NormalizedDraw) -> None:
    """Upsert a NormalizedDraw into the database.

    Searches for a Game by game_slug.  If not found, logs a warning and
    returns without raising.  Otherwise inserts or updates the matching
    Draw row (matched by game_id + draw_date + draw_type).
    """
